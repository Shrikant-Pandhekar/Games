...
def delete_command():
    DB_File.delete(selected_row[0])

def view_command():
    list.delete(0,END)
    for row in DB_File.view():
        list.insert(END,row)

def search_command():
    list.delete(0,END)
    for row in DB_File.search(date_text.get(),earning_text.get(),exercise_text.get(),study_text.get(),diet_text.get(),python_text.get()):
        list.insert(END,row)

def add_command():
    DB_File.insert(date_text.get(),earning_text.get(),exercise_text.get(),study_text.get(),diet_text.get(),python_text.get())
